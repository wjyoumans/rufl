...
use crate::Integer;
use flint_sys::{flint, fmpz, fmpz_mod_types, fmpz_mod};
use std::fmt;
use std::hash::{Hash, Hasher};
use std::mem::{ManuallyDrop, MaybeUninit};
use std::rc::Rc;

pub(crate) struct FmpzModCtx(fmpz_mod_types::fmpz_mod_ctx_struct);
...
impl IntModCtx {
    ...
    #[inline]
    pub fn as_ptr(&self) -> *const fmpz_mod_types::fmpz_mod_ctx_struct {
        &self.inner.0
    }
...
impl IntMod {
    ...
    #[inline]
    pub fn ctx_as_ptr(&self) -> *const fmpz_mod_types::fmpz_mod_ctx_struct {
        self.context().as_ptr()
    }
    ...
    #[inline]
    pub const unsafe fn from_raw(inner: flint::fmpz, ctx: IntModCtx) -> IntMod {
        IntMod { inner, ctx }
    }
